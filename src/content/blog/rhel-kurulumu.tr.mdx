...
faq:
  ...
  - q: 'RHEL''i gerçek bir bilgisayara kurabilir miyim, sanal makine şart mı?'
    a: 'Sanal makine şart değil; bu seride güvenli olduğu için sanal makine kullanıyoruz ama RHEL''i doğrudan gerçek bir bilgisayara da kurabilirsin. Tek fark başta: ISO''yu bir USB belleğe Fedora Media Writer, balenaEtcher, Rufus ya da dd ile yazarsın, sonra bilgisayarı BIOS/UEFI açılış menüsünden (çoğu makinede F12) o USB ile başlatırsın. Anaconda ekranları ikisinde de aynıdır. Dikkat: gerçek diskte kendi verin durur; Automatic bölümleme boş alanı kullanır ama kurulumdan önce mutlaka yedek al. Windows ile aynı diskte tutmak istiyorsan çift önyükleme (dual boot) gerekir, o daha ileri bir konudur.'
---
...
4. İki ISO var. **Boot ISO** (1 GB) yalnızca kurulum programını içerir; paketleri kurulum sırasında Red Hat'in
   ...
   yapmak en kısa yol. Yazının geri kalanı bu yola göre; DVD ISO seçtiysen tek fark, kaydı Adım 5'te terminalden
   yapmak.
...
## Alternatif: gerçek bir bilgisayara kurmak

Bu seri boyunca sanal makine kullanacağız; öğrenirken en güvenli yol bu, çünkü bir şeyi bozarsan anlık görüntüye
dönersin ve ev sahibi bilgisayarın hiç etkilenmez. Ama RHEL'i gerçek bir bilgisayara, donanımın üstüne doğrudan
kurmak da isteyebilirsin: eski bir dizüstünü Linux laboratuvarına çevirmek, tam performans almak ya da bir sunucu
kurmak için. O zaman iki şey değişir; kurulum ortamını bir **USB belleğe** yazarsın ve makineyi **BIOS/UEFI'den**
o USB'yle açarsın. Ondan sonrası, birazdan göreceğin Anaconda ekranlarının aynısıdır.

<Callout type="caution" title="Önce yedek: gerçek disk, gerçek veri">
Sanal makinede disk boştu; yanlış yapsan da kaybedecek bir şey yoktu. Gerçek bir bilgisayarda o disk senin
fotoğraflarının, belgelerinin, belki Windows'unun durduğu yer. Kurulum "Automatic" seçilince diskteki boş alanı
kullanmaya çalışır, ama diski tamamen silme seçeneği de bir tık ötede. Kurulumdan önce **önemli her şeyi başka bir
diske ya da buluta yedekle.** Buradaki adımlar, boş ya da gözden çıkarılmış bir bilgisayar içindir.
</Callout>

### 1. ISO'yu USB belleğe yazmak

İndirdiğin ISO bir disk kalıbıdır; USB'ye dosya gibi kopyalamak yetmez, "yazmak" (flash) gerekir. Boot ISO için
en az 4 GB, DVD ISO için 16 GB'lık bir USB yeter; işlem USB'deki her şeyi siler. Araçlar:

| Araç | Platform | Not |
| --- | --- | --- |
| **Fedora Media Writer** | Windows, macOS, Linux | En kolayı: ISO'yu ve USB'yi seç, Write. Red Hat ailesi için birebir. |
| **balenaEtcher** | Windows, macOS, Linux | Basit ve görsel; üç adımda yazar. |
| **Rufus** | Windows | Gelişmiş; sorarsa **DD Image** modunu seç. |
| **`dd`** | Linux, macOS | Komut satırı; en hızlısı ama en tehlikelisi. |

Komut satırını sevenler için `dd` (Linux). Önce `lsblk` ile USB'nin adını bul (ör. `/dev/sdb`); **yanlış diski
seçersen o diskin içindekileri silersin,** iki kez kontrol et:

```bash title="dd ile USB'ye yazmak (Linux)" showLineNumbers=false
lsblk
sudo dd if=rhel-10.2-x86_64-boot.iso of=/dev/sdb bs=4M status=progress oflag=sync
```

macOS'ta aynı iş, `diskutil list` ile diski bulup `sudo dd if=... of=/dev/rdiskN bs=4m` biçiminde yapılır.

### 2. USB'den açmak: BIOS/UEFI

Bilgisayarı kapat, USB'yi tak, aç ve açılış logosu görünürken **açılış (boot) menüsü** tuşuna bas. Tuş üreticiye
göre değişir; ekranda bir an "Boot Menu: F12" gibi bir yazı geçer:

| Üretici | Açılış menüsü | Firmware (setup) |
| --- | --- | --- |
| Dell, Lenovo (çoğu) | F12 | F2 |
| HP | F9 | Esc ya da F10 |
| Asus, MSI | F8 / F11 | Del |
| Acer | F12 | F2 |

Menüden USB belleğini seç; adında markası ya da "USB" geçer, genelde başında "UEFI:" olur. Tek seferlik menü
yoksa firmware'e girip **boot order** (açılış sırası) listesinde USB'yi diskin önüne al. Windows'ta bir kısayol
daha var: Ayarlar → Sistem → Kurtarma → "Şimdi yeniden başlat" → bir aygıttan başlat.

<Callout type="note" title="Secure Boot: genelde sorun değil">
Modern bilgisayarlarda **Secure Boot** açık gelir; yalnızca imzalı işletim sistemlerinin açılmasına izin verir.
RHEL'in önyükleyicisi Microsoft tarafından imzalandığı için çoğu makinede Secure Boot açıkken bile sorunsuz
kurulur. USB açılmaz ya da "not signed" benzeri bir uyarı görürsen, firmware'den Secure Boot'u geçici olarak
kapatıp tekrar dene.
</Callout>

### 3. Gerisi aynı

USB'den açılınca, aşağıda anlattığım GRUB menüsü gelir; **Install Red Hat Enterprise Linux 10.2**'yi seç. Buradan
sonra Anaconda ekranları sanal makinedekiyle **birebir aynıdır** ve bu yazının geri kalanını olduğu gibi takip
edebilirsin. Tek gerçek fark **Kurulum Hedefi (Adım 7)** ekranında: orada "ATA VBOX HARDDISK" değil, kendi
diskin görünür. Yalnızca RHEL kuracaksan doğru diski seçip "Automatic" bırak; diskte kalması gereken başka bir
sistem (ör. Windows) varsa iş **çift önyükleme (dual boot)** alanına girer, o daha ileri bir konudur ve ayrı bir
yazıyı hak ediyor. Kurulum bitince USB'yi çıkar ki bilgisayar diskten açılsın.

## Adım 4: Anaconda ile kurulum

Sanal makineyi **Start** ile başlat (gerçek bir bilgisayarda: USB'den açtığında aynı menü gelir). ISO'dan açılır
ve birkaç saniye içinde bir menü gelir. RHEL'in kurulum
...
5. **Red Hat'e bağlan (Connect to Red Hat).** Boot ISO ile bu adım **zorunlu:** kayıt yapılmadan Software
   ...
   ile kurarken bu kutuyu atlayıp kaydı sonradan terminalden yapabilirsin; Adım 5'te gösteriyorum.)
...
7. **Kurulum hedefi (Installation Destination).** Tek disk görünür: "ATA VBOX HARDDISK" adlı sanal diskimiz
   (bende 87,7 GiB), üstünde onay işareti. Gerçek bir bilgisayarda burada birden çok disk görebilirsin; doğru
   olanı seçmek kritik (yukarıdaki *Alternatif: gerçek bir bilgisayara kurmak* bölümüne bak). **Storage
   Configuration: Automatic** kalsın. Altta bir de **Encrypt my data** kutusu var; Bölüm 1'in
